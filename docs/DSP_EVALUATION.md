...
Follow-up candidate (`2026-09-12`): an offline-only 4x detector with 64 taps per phase, a look-ahead hold, and conservative-bound pruning reduces over-full-scale estimates from 30/51 production cases to 0/51 in the combined original/expanded fixture sets. The smallest observed margin is about 0.475 dB. Both reconstructors now run even when the short one passes. This is not promoted into the extension: browser cost, control-transition stress and controlled listening remain gates. Implementation, reproducible commands and evidence boundaries are in [TRUE_PEAK_CANDIDATE.md](TRUE_PEAK_CANDIDATE.md).
