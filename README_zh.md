...
请用网站播放器或操作系统音量设置整体聆听音量。LoudEase 的两个滑条只控制压低响亮片段、抬升安静细节的力度。项目有意不再增加单独的“整体音量”滑条：它只会重复已有音量控制，并不能改善不同视频、直播或片段之间的响度差异。

| 压住突兀的大声 | 安全提起小声细节 | 尊重原有控制 |
|---|---|---|
| 快速增益衰减与前视限幅共同接住持续大声和短促峰值。 | 节目基线负责抬起整体偏小的来源；清晰可听的节目内部小声细节最多额外提升 16 dB，近静音底噪不会被追着放大。 | 静音和播放器零音量始终是硬边界；只有收到新鲜运行证据时，界面才显示“生效中”。 |
...
用户授权后，LoudEase 会把标签页作为一条完整音频流捕获，并在本机 `AudioWorklet` 中处理。K-weighted 门控节目响度建立稳定基线；有声门槛与 16 dB 上限补充清晰可听的小声细节；独立快速保护与 5 ms 前视限幅接住突然的大声。同一页面内临时 `blob` 或 `srcObject` 替换不会重置节目。向上提升仍要等到有代表性的信号，但连续有效音频约 1.5 秒即可达到完整置信，避免短小声视频大半段都没有被提起来。原始音频不会上传。
...
商店版为了隐私和审核更精简，但不会使用更弱的平衡算法。`dist/github-dev` 只增加贡献者诊断与测试入口；两种构建都遵守 Chrome 的标准用户授权流程，也不会修改 Chrome 快捷方式。公开 GitHub Release 附带的是已剥离商店 ZIP，`dist/github-dev` 不作为普通公开安装包。
