...
const publicBetaStatus = fs.readFileSync(path.join(root, 'docs', 'PUBLIC_BETA_STATUS.md'), 'utf8');
const publicBetaReleaseNotes = fs.readFileSync(path.join(root, 'docs', 'releases', '0.8.2-beta.1.md'), 'utf8');
const storeListing = fs.readFileSync(path.join(root, 'store', 'STORE_LISTING.md'), 'utf8');
const privacyPractices = fs.readFileSync(path.join(root, 'store', 'PRIVACY_PRACTICES.md'), 'utf8');
...
const checks = [
  ...
  ['store dashboard copy and privacy fields exist', ['store/STORE_LISTING.md', 'store/PRIVACY_PRACTICES.md', 'store/ASSETS.md'].every((file) => fs.existsSync(path.join(root, file))) && /Single purpose/.test(privacyPractices)],
  ['published privacy guidance no longer claims the repository is private', /Dashboard review/.test(privacyPractices) && !/current private GitHub repository/.test(privacyPractices)],
  ['store screenshots and promotional tile use exact dimensions', pngHasSize('store/assets/screenshot-balancing-1280x800.png', 1280, 800) && pngHasSize('store/assets/screenshot-settings-1280x800.png', 1280, 800) && pngHasSize('store/assets/screenshot-onboarding-1280x800.png', 1280, 800) && pngHasSize('store/assets/promo-small-440x280.png', 440, 280)],
  ...
  ['0.8.2 GitHub prerelease notes pin source archive checksum and evidence boundary', /c13a0d42793f412d19bab7af79a2400017a57c1b/.test(publicBetaReleaseNotes) && /6C212FAEAC5D428A878CCAEAA7BDAF611BFB999F60E8387CF90DCB9B351422F9/.test(publicBetaReleaseNotes) && /remain outstanding/.test(publicBetaReleaseNotes)],
  ['readmes link current privacy and release limits', /PRIVACY\.md/.test(readme) && /RELEASE_READINESS_REVIEW\.md/.test(englishReadme)],
  ['public beta adoption snapshot distinguishes installs from active use', /Snapshot date:/.test(publicBetaStatus) && /not evidence that four people actively used/.test(publicBetaStatus) && /Users report captures installations/.test(publicBetaStatus) && /0` confirmed independent external users/.test(publicBetaStatus) && /all available Chrome Web Store regions/.test(publicBetaStatus)],
  ['public beta status records current GitHub distribution and store experiment', /`v0\.8\.2-beta\.1` prerelease are public/.test(publicBetaStatus) && /574,617/.test(publicBetaStatus) && /waiting for Chrome Web Store review/.test(publicBetaStatus) && /GA4 listing analytics was enabled/.test(publicBetaStatus)],
  ...
  ['community support routes reports through live public channels', /Security vulnerability/.test(support) && /Store Support Hub/.test(support) && /public GitHub Issue Forms/.test(support) && /WSL043/.test(support) && !/will be enabled only/.test(support)],
  ...
  ['release documentation records durable public beta history', /one public GitHub beta/.test(publishing) && /Public beta releases are durable history/.test(publishing) && /explicit maintainer approval/.test(agentGuide) && /require explicit authorization/.test(publishing)],
  ['store analytics wording preserves the extension telemetry boundary', /Extension analytics or background telemetry: No/.test(storeListing) && /Google-managed GA4 enabled/.test(storeListing) && /no GA4 or other analytics code is bundled/.test(privacyPractices)],
  ...
  ['Chrome Web Store account checklist is documented', /one-time developer registration fee/.test(accountSetup) && /two-step verification/.test(accountSetup) && /require explicit authorization/.test(publishing)],
