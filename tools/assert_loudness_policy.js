...
const checks = [
  ...
  ['source-domain player-volume compensation does not alter captured peak headroom', /sourceSample = sample \* this\.sourceVolumeGain/.test(worklet) && /weightedSample\(sourceSample/.test(worklet) && /controlInput\.peakDb = linearToDb\(this\.inputPeak\)/.test(worklet)],
  ...
  ['pause and resume retain the last active programme identity', /let lastProgrammeKey = ''/.test(bridge) && /audibleSources\.length > 0/.test(bridge) && /href !== lastProgrammeHref/.test(bridge) && /const lastProgrammeKeys = new Map\(\)/.test(background) && /lastProgrammeKeys\.set\(tabId, programmeKey\)/.test(background)],
  ...
  ['muted auxiliary media cannot redefine the audible programme', /\.filter\(isAudible\)\s*\.map\(programmeSourceIdentity\)/.test(bridge)],
