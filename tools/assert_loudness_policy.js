...
const checks = [
  ...
  ['cold-start lift is confidence-gated', /confidenceStartBlocks: 1/.test(policy) && /confidenceFullBlocks: 9/.test(policy) && /\* confidence/.test(policy)],
  ...
  ['adaptive transition ceiling follows learned output peak', /transitionAllowanceDb: 3/.test(policy) && /recentOutputPeakDb/.test(policy) && /computeTransitionCeilingDb/.test(worklet)],
  ...
  ['media stream replacement gets a local-only source identity', /const sourceObjectIds = new WeakMap\(\)/.test(bridge) && /function programmeSourceIdentity\(media\)/.test(bridge) && /sourceObjectIds\.get\(media\.srcObject\)/.test(bridge)],
