...
const checks = [
  ...
  ['full lift range supports strong leveling', Number.isFinite(maxLiftDb) && maxLiftDb === 34],
  ['main loudness target remains bounded', Number.isFinite(targetRmsDb) && targetRmsDb === -29],
  ...
  ['cut and lift use separate attack and release envelopes', Number.isFinite(cutAttackSeconds) && Number.isFinite(cutReleaseSeconds) && Number.isFinite(liftAttackSeconds) && Number.isFinite(liftReleaseSeconds) && cutAttackSeconds < liftAttackSeconds && cutAttackSeconds < cutReleaseSeconds && liftAttackSeconds < liftReleaseSeconds],
  ['upward recovery is strong but bounded', Number.isFinite(maxGainIncreaseStepDb) && maxGainIncreaseStepDb === 3],
  ...
  ['offscreen strong lift policy is explicit', /maxLiftDb: 34/.test(core) && /liftTargetRmsDb: -29/.test(core) && /liftLimiterBudgetDb: 15/.test(core) && /const MAX_LIFT_DB = 34;/.test(offscreen) && /const LIFT_LIMITER_BUDGET_DB = 15;/.test(offscreen)],
  ['limiter-bound quiet output uses damped measured assistance', /realizedLiftAssistRatio: 0\.5/.test(core) && /outputTargetRmsDb/.test(core) && /limiterReductionDb/.test(core) && /realizedLiftAssistDb/.test(offscreen)],
  ['offscreen lift remains smoothed', /const LIFT_ATTACK_SECONDS = 0\.1(?:0)?;/.test(offscreen) && /const LIFT_RELEASE_SECONDS = 0\.25;/.test(offscreen)],
  ['offscreen upward gain step is bounded', Number.isFinite(maxGainIncreaseStepDb) && maxGainIncreaseStepDb === 3 && /frameStartGainDb \+ MAX_GAIN_INCREASE_STEP_DB/.test(offscreen)],
  ['offscreen lift uses a robust fast loudness window and robust peak window', /LIFT_LOUDNESS_HISTORY_FRAMES = 5/.test(offscreen) && /LIFT_CONTROL_HISTORY_FRAMES = 10/.test(offscreen) && /LIFT_LOUDNESS_PERCENTILE = 0\.5/.test(offscreen) && /LIFT_PEAK_PERCENTILE = 0\.65/.test(offscreen) && /energyToDb\(percentileLast\(this\.energyHistory, LIFT_LOUDNESS_HISTORY_FRAMES, LIFT_LOUDNESS_PERCENTILE\)\)/.test(offscreen) && /percentileLast\(this\.peakHistory, LIFT_CONTROL_HISTORY_FRAMES, LIFT_PEAK_PERCENTILE\)/.test(offscreen) && /liftPeakDb/.test(offscreen)],
  ...
  ['offscreen target has deadband and upward hold', /stabilizeGainTarget/.test(core) && /TARGET_DEADBAND_DB = 0\.8/.test(offscreen) && /TARGET_HOLD_MS = 80/.test(offscreen) && /targetHoldCount/.test(offscreen)],
  ['fast loudness strengthens protective cut while lift uses its own window', /fastCutEnergy/.test(offscreen) && /Math\.max\(loudness\.controlDb, energyToDb\(fastCutEnergy\)\)/.test(offscreen) && /this\.updateGain\(controlInputDb, liftControlInputDb/.test(offscreen)],
  ...
  ['player-volume compensation does not double-count peak headroom', /const liftRmsDb = liftWindowRmsDb \+ volumeCompensationDb;/.test(offscreen) && /liftPeakDb,[\s\S]*?settings: this\.settings/.test(offscreen) && !/adjustedLiftPeakDb/.test(offscreen)],
  ...
  ['shared DSP separates peak headroom and bounded limiter allowance', /liftPeakDb/.test(core) && /peakHeadroomDb = limiterCeilingDb - liftPeakDb - liftHeadroomReserveDb/.test(core) && /rawPeakHeadroomDb = limiterCeilingDb - peakDb - 0\.5/.test(core) && /effectiveLiftBudgetDb = effectiveLiftHeadroomDb \+ liftLimiterBudgetDb/.test(core)],
