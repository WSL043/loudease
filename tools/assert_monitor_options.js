...
const checks = [
  ...
  ['monitor saves custom strength and target overrides for each site', /type: 'WVB_SAVE_SITE_SETTINGS'/.test(monitorJs) && /preset: 'custom'/.test(monitorJs) && /cutStrength: clampPercent\(els\.siteCut\.value\)/.test(monitorJs) && /liftStrength: clampPercent\(els\.siteLift\.value\)/.test(monitorJs) && /targetLoudnessDb: clampTargetLoudness\(els\.siteTarget\.value\)/.test(monitorJs)],
  ['legacy site rules inherit the global target in the editor', /fillSiteForm\(siteKey, \{ \.\.\.\(lastOptionsState\?\.settings \|\| \{\}\), \.\.\.\(lastOptionsState\?\.siteSettings\?\.\[siteKey\] \|\| \{\}\) \}\)/.test(monitorJs)],
