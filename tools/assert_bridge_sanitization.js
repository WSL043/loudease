...
const checks = [
  ...
  ['bridge prunes detached media listeners', /function pruneMediaListeners\(\)/.test(bridge) && /media\.isConnected/.test(bridge) && /observedMedia\.delete\(media\)/.test(bridge)],
  ['bridge observes media without WebAudio', /function scanMedia\(\)/.test(bridge) && /document\.createTreeWalker\(root, NodeFilter\.SHOW_ELEMENT\)/.test(bridge) && !/AudioContext|createMediaElementSource|createMediaStreamSource/.test(bridge)],
  ...
  ['bridge scans open shadow roots with a bounded budget', /MAX_SCAN_NODES/.test(bridge) && /MAX_MEDIA_ITEMS/.test(bridge) && /element\.shadowRoot/.test(bridge)],
  ...
  ['bridge sends frame status only from own scan', /sendRuntime\(\{ type: 'WVB_FRAME_STATUS', status: buildStatus\(\) \}\)/.test(bridge)],
