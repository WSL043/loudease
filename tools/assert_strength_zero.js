...
const checks = [
  ...
  ['shared DSP zero strength returns unity gain', /Math\.max\(cutScale, liftScale\) <= 0/.test(policy) && /output\.targetGainDb = 0/.test(policy)],
  ['shared DSP scales cut and lift independently', /const cutScale = percent\(input\.cutStrength, 100\) \/ 100/.test(policy) && /const liftScale = percent\(input\.liftStrength, 100\) \/ 100/.test(policy) && /maximumCutDb = options\.maxCutDb \* cutScale/.test(policy) && /maximumLiftDb = input\.canLift === false \? 0 : options\.maxLiftDb \* liftScale/.test(policy)],
