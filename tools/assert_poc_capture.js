...
const checks = [
  ['offscreen pipeline is leveler-v3', /const PIPELINE_MODE = 'leveler-v3';/.test(offscreen)],
  ...
  ['background aggregates capture track and context state', /captureContextState/.test(background) && /captureAudioTrackCount/.test(background) && /capturePipelineMode/.test(background)],
